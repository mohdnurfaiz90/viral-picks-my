const siteConfig = {
  ...
  heroImage: "exec-2e0d79d6-74f6-43ab-9cd1-deba06ced1c1.png",
  platforms: [
    { id: "tiktok", name: "TikTok Shop", icon: "tiktok" },
    { id: "shopee", name: "Shopee", icon: "shopee" },
  ],
  products: Array.isArray(window.showroomProducts) ? window.showroomProducts : [],
};
...
function renderHome() {
  ...
  app.innerHTML = `
    <section class="home" aria-labelledby="site-title">
      ...
      <p class="home-footer">© ${new Date().getFullYear()} ${siteConfig.name}</p>
    </section>`;
...
function renderProducts() {
  ...
  rows.innerHTML = matches.map((product) => {
    ...
    return `<tr>
      ...
      <td><div class="product-info"><div class="product-image"><img src="${product.image}" alt="Gambar ${product.name}" loading="lazy" referrerpolicy="no-referrer" /></div><div><span class="product-name">${product.name}</span><span class="product-sub">${product.subcategory || product.category}</span></div></div></td>
